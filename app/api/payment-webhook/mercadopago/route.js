...
export async function POST(request){
 ...
 try{
  ...
  const product=getDigitalProductByReference(payment.externalReference);if(!product)return NextResponse.json({received:true,authenticated:true,verified:true,error:'product_reference_not_found',reference:payment.externalReference,deliveryCreated:false},{status:409});
  ...
  const sale=buildConfirmedSale(payment,{productId:product.id,price:product.price});const saved=saveSaleEvent(sale);const delivery=createDelivery({event:saved.event,asset:product.asset});
  let email={sent:false,error:'buyer_email_required'};if(delivery?.buyerEmail){try{const message=buildDeliveryMessage({delivery,productTitle:product.title});email=await sendTransactionalEmail(message);if(email.sent)delivery.email={status:'sent',provider:email.provider,id:email.id,sentAt:email.sentAt};}catch(e){email={sent:false,error:e.message}}}
