...
export async function searchPublicWeb(query) {
  ...
  return { configured: true, results: raw.slice(0, 20).map(item => normalizeExternalResult({ title: item.title, snippet: item.snippet || item.description, url: item.url || item.link, source: 'public-web' })) };
}
